...
def joint_plan(image,scene,rules):
    ...
    for i,rule in enumerate(rules):
        ...
        distances/=max(.6 if rule['exact'] else rule['tolerance'],.001)
    ...
    if candidates:
        ...
        with np.errstate(divide='ignore',invalid='ignore'):
            ...
            worst=np.maximum(c1[:,None],c2[None,:])
            for marker,cost in maps[2:]:
                ...
                worst=np.maximum(worst,cost[np.clip(sy,0,h-1),np.clip(sx,0,w-1)])
            rank=worst+0.00001*(abs(angles)+abs(np.log(np.maximum(scales,.001)))*100)
            rank[~valid]=np.inf
            if np.isfinite(rank).any():
                j=np.unravel_index(np.argmin(rank),rank.shape)
                geometric=dict(score=float(worst[j]),dx=float(translation.real[j]),dy=float(translation.imag[j]),angle=float(angles[j]),scale=float(scales[j]))
    ...
    def evaluate(angle,scale,dx,dy):
        ...
        shifts=np.column_stack((dx,dy));worst=np.zeros(len(dx));total=np.zeros(len(dx))
        for marker,cost in maps:
            ...
            worst=np.maximum(worst,values);total+=values
        # Every region must pass. Mean distance only breaks ties.
        rank=worst+total*.0001;j=int(np.argmin(rank))
        return float(worst[j]),int(dx[j]),int(dy[j])
    ...
    for scale in (1.,.85,.925,1.075,1.15,1.225):
        for angle in (0.,-10.,10.,-20.,20.,-30.,30.,-40.,40.):
            score,x,y=evaluate(angle,scale,dx,dy)
            if np.isfinite(score):seeds.append(dict(score=score,dx=x,dy=y,angle=angle,scale=scale))
            if np.isfinite(score) and (best is None or score<best['score']):
                best=dict(score=score,dx=x,dy=y,angle=angle,scale=scale)
    if best is None:return None
    # Retain several basins: sharp color islands make a single coarse winner unreliable.
    for seed in sorted(seeds,key=lambda p:p['score'])[:6]:
        ...
        for scale in np.arange(seed['scale']-.04,seed['scale']+.041,.01):
            for angle in np.arange(seed['angle']-5,seed['angle']+5.1,1.):
                score,x,y=evaluate(angle,scale,xx.ravel(),yy.ravel())
                if score<best['score']:best=dict(score=score,dx=x,dy=y,angle=float(angle),scale=float(scale))
    yy,xx=np.mgrid[best['dy']-4:best['dy']+5,best['dx']-4:best['dx']+5]
    score,x,y=evaluate(best['angle'],best['scale'],xx.ravel(),yy.ravel())
    best.update(score=score,dx=x,dy=y)
    return geometric if geometric is not None and geometric['score']<best['score'] else best
