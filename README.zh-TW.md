...
按 F8 後最多等待60秒，可先從背包進入一般染色並完成教學；辨識到色板與倒數計時後自動開始。等待期間不操作滑鼠，按 F9 可取消；逾時會彈窗提示重新按 F8。

介面會依視窗大小調整配置：寬視窗並排三區，窄視窗以區域頁籤切換。所有工具介面控制項均不接受滾輪輸入；數值以點擊或拖曳調整，極小視窗可拖曳捲軸查看下方設定。
